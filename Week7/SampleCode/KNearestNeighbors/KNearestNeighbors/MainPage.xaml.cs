...
namespace KNearestNeighbors
{
    public partial class MainPage : PhoneApplicationPage
    {
        ...
        double calcDistance(float[] a, float[] b)
        {
            ...
            // We take the first element, which is accelerometer, and just do Euclidean Distance for 1-dimension
            //result += Math.Abs(a[0] - b[0]);

            // We take the second element, which is an angle, and find the angle difference between the two:
            //double diff = a[1] - b[1] % (4*Math.PI);
            //result += ((diff + 2*Math.PI) % 4 * Math.PI);
            return result;
        }
        ...
        private async Task preRecordBeeps()
        {
            // Beep low
            await doBeep(880, 500);
            ...
            // Beep high to signify recording has started, but don't await this one, so we start recording simultaneously
            doBeep(1760, 500);
        }

        private void discardTailEnd(List<float[]> data)
        {
            // Since we sample 10 times a second, discard the last 4 data points:
        ...
        private async void recPosButton_Click(object sender, RoutedEventArgs e)
        {
            if (recordingPositive)
            {
                ...
                recPosButton.Content = "Record Positive Example";

                // Stop recording
                recordingPositive = false;

                // Cut off the last half second of data, it's probably corrupt
                discardTailEnd(posExamples);

                // See if we can enable classification now
                updateClassifyButton();
            }
            else
            {
                recNegButton.IsEnabled = false;
                recPosButton.Content = "Stop";

                // Do the series of beeps needed to tell the user to start recording!
                await preRecordBeeps();
        ...
        private async void recNegButton_Click(object sender, RoutedEventArgs e)
        {
            if (recordingNegative)
            {
                ...
                recNegButton.Content = "Record Negative Example";

                // Stop recording
                recordingNegative = false;

                // Cut off the last half second of data, it's probably corrupt
                discardTailEnd(negExamples);
